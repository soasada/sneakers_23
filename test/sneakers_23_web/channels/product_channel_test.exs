defmodule Sneakers23Web.ProductChannelTest do
  ...
  describe "notify_item_stock_change/1" do
    setup _ do
      {inventory, _data} = Test.Factory.InventoryFactory.complete_products()
      [product = %{items: [item]}, _] = Sneakers23.Inventory.CompleteProduct.get_complete_products(inventory)
      topic = "product:#{product.id}"
      Endpoint.subscribe(topic)
      {:ok, %{product: product, item: item}}
    end

    test "the same stock level does not broadcast an event", %{item: item} do
    ...
    test "a stock level change broadcasts an event", %{product: product, item: item} do
